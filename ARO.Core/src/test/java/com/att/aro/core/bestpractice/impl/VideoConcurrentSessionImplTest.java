...
public class VideoConcurrentSessionImplTest extends BaseTest{
	...
	private VideoConcurrentSessionImpl videoConcurrentSessionImpl;
	...
	@Before
	public void setup() {
		videoConcurrentSessionImpl = (VideoConcurrentSessionImpl) context.getBean("videoConcurrentSession");
	...
	@Test
	public void runTest_two_manifests_one_selected_AND_no_concurrancy_SELF_TEST() {
		...
		result = (VideoConcurrentSessionResult) videoConcurrentSessionImpl.runTest(tracedata);

		assertThat(result.getResultType()).isSameAs(BPResultType.SELF_TEST);
		assertThat(result.getResultText()).contains("There were no concurrent sessions in this manifest.");

		assertThat(result.getAboutText()).contains("Using too many concurrent sessions may cause problems with smooth delivery of content.");
		assertThat(result.getDetailTitle()).contains("Concurrent Session");
		assertThat(result.getOverviewTitle()).contains("Video: Concurrent Session");
	}

	@Test
	public void runTest_two_manifests_selected_CONFIG_REQUIRED() {
	...
	@Test
	public void runTest_two_invalid_manifests_selected_CONFIG_REQUIRED() {
