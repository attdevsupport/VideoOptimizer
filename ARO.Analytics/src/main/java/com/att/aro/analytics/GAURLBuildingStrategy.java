...
public class GAURLBuildingStrategy implements URLBuildingStrategy{
    ...
    //private static final Logger LOGGER = Logger.getLogger(GAURLBuildingStrategy.class.getName());
...
//        LOGGER.info("Application Name : "+this.appName + " Encoded : " +encode(this.appName));
    ...
    private String getExceptionParams(){
        ...
        if (encode(this.appName)!=null && encode(this.appName).length()>0) {
        	...
        	appParams.append(appendString(GACommonParameter.applicationversion.param(),  "6.0.0"));        	
        }
        ...
        if(aPoint != null){
        	...
        	if (checkNotForNull(aPoint.getExceptionDesc())){
        		if (aPoint.getDataSource()!=null && GACommonParameter.datasource.param()!=null) 
        			paramsUrls.append(appendString(GACommonParameter.datasource.param(), aPoint.getDataSource().replace(" ", "%20")));
        		if (aPoint.getExceptionDesc()!=null && GACommonParameter.exceptiondesc.param()!=null)
        			paramsUrls.append(appendString(GACommonParameter.exceptiondesc.param(), aPoint.getExceptionDesc().replace(" ", "%20")));
        		if (aPoint.isFatal()) {
