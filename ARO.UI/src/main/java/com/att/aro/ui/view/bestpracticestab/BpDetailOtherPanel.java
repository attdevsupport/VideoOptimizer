...
public class BpDetailOtherPanel extends BpDetail {
	...
	public BpDetailOtherPanel(String title,
		...
		addPanel(row++, new BpDetailItem("other.accessingPeripherals", BestPracticeType.ACCESSING_PERIPHERALS));
		
		fullPanel.add(dataPanel, BorderLayout.CENTER);
		fullPanel.add(detailPanel, BorderLayout.SOUTH);
		add(fullPanel);
		
		List<BestPracticeType> list = BestPracticeType.getByCategory(Category.OTHER);
