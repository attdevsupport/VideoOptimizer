...
//		ApplicationContext context = new AnnotationConfigApplicationContext(AROConfig.class);
	...
	private void setProfile(Profile chosenProfile) {
		...
		synchronized(factory){
			...
			setTitle(MessageFormat.format(DEFAULTBUNDLE.getString("configuration.title"), 
	...
	private void saveProfile(File saveToFile, Profile profile)
			throws IOException {
		// FIXME: re-factor for 6.1 release
		if (profile != null && saveToFile != null) {
			String filename = saveToFile.getAbsolutePath();
