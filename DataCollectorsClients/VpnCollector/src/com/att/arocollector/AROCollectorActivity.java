...
import com.att.arotcpcollector.util.DateUtils;
...
public class AROCollectorActivity extends Activity {
	...
	private ComponentName vpnServiceIntent;
	...
	protected void onCreate(Bundle savedInstanceState) {
	    ...
	    videoOption = videoOptionTmpEnum == null? videoOption : videoOptionTmpEnum;	    
	    ...
	    screenSize = screenSizeTmp == null? screenSize : screenSizeTmp;
 	    ...
 	    Log.i(TAG, " video: " + videoOption + " bitRate: " + bitRate + " screenSize: " + screenSize);
		...
		{ // test code
			...
			boolean valu = (packageInfo.applicationInfo.flags & ApplicationInfo.FLAG_DEBUGGABLE) != 0;
			   // build datetime
            String appBuildDate = DateUtils.getDate("yyyy/MM/dd hh:mm",
...
//		saveLogCat();
	...
	private void saveLogCat(){
		try {
			...
			String fileName = "logcat_"+getDateTime()+".txt";
		    File outputFile = new File("/sdcard/ARO/",fileName);
		    @SuppressWarnings("unused")
		    Process process = Runtime.getRuntime().exec("logcat -f "+outputFile);
		    } catch (IOException e) {
		...
		switch (requestCode) {
			...
			case Config.Permission.VPN_PERMISSION_REQUEST_CODE:
				if (resultCode == RESULT_OK) {
					...
					captureVpnServiceIntent.addCategory(CaptureVpnService.SERVICE_CLOSE_CMD_INTENT);
					...
					vpnServiceIntent = startService(captureVpnServiceIntent);
					vpnStatus = true;
			...
			case Config.Permission.VIDEO_PERMISSION_REQUEST_CODE:
	            ...
	            videoCapture = new VideoCapture(getWindowManager(),
	            			mediaProjection, bitRate, screenSize);
	            videoCapture.start();
				break;
...
//		FileUtil.terminate();
	...
	private boolean isAirplaneModeOn() {
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) {
			Log.i(TAG, "AIR plane mode message "
	...
	private boolean networkAndAirplaneModeCheck() {
		 ...
		 * 
		 * message =
		 * "Your phone is in Airplane Mode and ARO doesn't support Airplane mode"
		 * ; //popup dialog showInfoDialog(title, message); networkChecker =
		 * false; } else
		 */
	...
	public void handleUncaughtException (Thread thread, Throwable e)  {
		FileUtil.print("SSL1.txt", "UNCAUGHT, Class: " + e.getClass() + " ,  Throwable Cause: " + e.getCause() + " , thread name: " + thread.getName() + "\n");
		FileUtil.print("SSL1.txt", "UNCAUGHT " + thread.getStackTrace()[0] + "\n");
		FileUtil.print("SSL1.txt", "UNCAUGHT " + thread.getStackTrace()[1] + "\n");
		FileUtil.print("SSL1.txt", "UNCAUGHT " + thread.getStackTrace()[2] + "\n");
		FileUtil.print("SSL1.txt", "UNCAUGHT " + thread.getStackTrace()[3] + "\n");
		FileUtil.print("SSL1.txt", "UNCAUGHT " + thread.getStackTrace()[4] + "\n");
		FileUtil.print("SSL1.txt", "UNCAUGHT " + thread.getStackTrace()[5] + "\n");

		MemoryInfo mi = new MemoryInfo();
		ActivityManager activityManager = (ActivityManager)this.getSystemService(Activity.ACTIVITY_SERVICE);
		activityManager.getMemoryInfo(mi);
		long availableMegs = mi.availMem / 1048576L;
		FileUtil.print("SSL1.txt", " ServerBuff : Available memory: "+ availableMegs + "\n\n");
	}
    ...
    boolean extractResource(File filepath, int rsrc) {
		...
		try {
			iStream = this.getResources().openRawResource(rsrc);
			if (filepath.exists()) {
				filepath.delete();
			}
