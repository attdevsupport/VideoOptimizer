 ...
 * objects.
 */
public class ImageComparatorTableModel extends DataTableModel<ImageMdataEntry> {

	private static final long serialVersionUID = 1L;

	private static final Logger LOGGER = Logger.getLogger(ImageComparatorTableModel.class.getName());
	...
	private static final int COL3_MIN = 165;
	private static final int COL3_MAX = 200;
	private static final int COL3_PREF = 165;
	...
	private static final String[] COLUMNS = { ResourceBundleHelper.getMessageString("uiComparator.table.col1"),
											  ResourceBundleHelper.getMessageString("uiComparator.table.col2"),
											  ResourceBundleHelper.getMessageString("uiComparator.table.col3"),
											  ResourceBundleHelper.getMessageString("uiComparator.table.col4"),
											  ResourceBundleHelper.getMessageString("uiComparator.table.col5")};

	/**
	 * Initializes a new instance of the ImageComparatorTableModel.
	 */
	public ImageComparatorTableModel() {
		super(ImageComparatorTableModel.COLUMNS);
	}
	...
	public TableColumnModel createDefaultTableColumnModel() {
		...
		col = cols.getColumn(ImageComparatorTableModel.COL_1);
		col.setCellRenderer(new NumberFormatRenderer(new DecimalFormat("0.000")));
		col.setMinWidth(ImageComparatorTableModel.COL1_MIN);
		col.setPreferredWidth(ImageComparatorTableModel.COL1_PREF);
		col.setMaxWidth(ImageComparatorTableModel.COL1_MAX);

		col = cols.getColumn(ImageComparatorTableModel.COL_2);
		col.setMinWidth(ImageComparatorTableModel.COL2_MIN);
		col.setPreferredWidth(ImageComparatorTableModel.COL2_PREF);
		//col.setMaxWidth(ImageCompressionTableModel.COL2_MAX);

		col = cols.getColumn(ImageComparatorTableModel.COL_3);
		col.setMinWidth(ImageComparatorTableModel.COL3_MIN);
		col.setPreferredWidth(ImageComparatorTableModel.COL3_PREF);
		col.setMaxWidth(ImageComparatorTableModel.COL3_MAX);

		col = cols.getColumn(ImageComparatorTableModel.COL_4);
		col.setMinWidth(ImageComparatorTableModel.COL4_MIN);
		col.setPreferredWidth(ImageComparatorTableModel.COL4_PREF);
		col.setMaxWidth(ImageComparatorTableModel.COL4_MAX);
		
		col = cols.getColumn(ImageComparatorTableModel.COL_5);
		col.setMinWidth(ImageComparatorTableModel.COL5_MIN);
		col.setPreferredWidth(ImageComparatorTableModel.COL5_PREF);
		col.setMaxWidth(ImageComparatorTableModel.COL5_MAX);
		return cols;
	}
	...
	public Class<?> getColumnClass(int columnIndex) {
		ImageComparatorTableModel.LOGGER.log(Level.FINE, "getColumnClass, idx: {0}", columnIndex);
		switch (columnIndex) {
		case COL_1:
			return Double.class;
		case COL_2:
	...
	protected Object getColumnValue(ImageMdataEntry item, int columnIndex) {
		ImageComparatorTableModel.LOGGER.log(Level.FINEST, "getColumnValue, idx:{0}", columnIndex);
		switch (columnIndex) {
		case COL_1:
		...
		case COL_3:
			return item.getImgSize();
		case COL_4:
			return item.getFormatdSize();
		case COL_5:
