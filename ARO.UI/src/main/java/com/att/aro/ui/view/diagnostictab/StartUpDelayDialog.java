...
public class StartUpDelayDialog extends JDialog {
	...
	private VideoSegmentAnalyzer videoSegmentAnalyzer = ContextAware.getAROConfigContext().getBean("videoSegmentAnalyzer", VideoSegmentAnalyzer.class);
	...
	class ComboManifest {
		...
		public ComboManifest(VideoStream videoStream) {
			if (videoStream.isSelected()) {
				...
				this.manifestName = videoStream.getManifest().getVideoName() + ", " + resourceBundle.getString("sliderdialog.manifest.segmentcount")
	...
	public StartUpDelayDialog(GraphPanel parentPanel, double maxVideoTime, VideoStream videoStream) {
		this(parentPanel, maxVideoTime, null, 0, new ArrayList<VideoEvent>(videoStream.getVideoEventList().values()));
		if (startTime == 0) {
	...
	public StartUpDelayDialog(GraphPanel parentPanel, double maxVideoTime, Map<Integer, VideoEvent> chunks, int indexKey, List<VideoEvent> allChunks) {
		...
		chunkSelectionList = chunks != null ? chunks : new TreeMap<>();
	...
	public double loadStartupDelay() {
		...
		VideoStreamStartup videoStreamStartup = ((TraceDirectoryResult) traceData.getAnalyzerResult().getTraceresult()).getVideoStartup();
	...
	public void showWarningMessage() {
		JOptionPane.showMessageDialog(StartUpDelayDialog.this, "Please select a segment and a manifest first in order to set the start up delay.", "Warning",
	...
	public JTextField getStartTimeDisplay() {
		...
		startTimeField.addActionListener(startTimeTextFieldActionListener(startTimeField));
		startTimeField.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == 157) { // mac Command key
					updateStartTime(startTimeField);
				}
			}
		});
	...
	public void updateStartTime(JTextField startTimeField) {
		double tempTime;
		try {
			...
			LOG.debug("Failed to parse bad data in startTimeField:" + startTimeField.getText());
			tempTime = getStartTime();
		}
		setStartTime(tempTime);
	}

	public ActionListener startTimeTextFieldActionListener(JTextField startTimeField) {
		return new ActionListener() {
	...
	public JButton getTunerButton(String btnName) {
		...
		tunerBtn.addActionListener(tunerButtonActionListener());
		return tunerBtn;
	}
