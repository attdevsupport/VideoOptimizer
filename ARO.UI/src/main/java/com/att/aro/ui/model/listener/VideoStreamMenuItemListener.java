...
public class VideoStreamMenuItemListener extends AbstractMenuItemListener {
    ...
    private static final List<String> videoColumnsToSkip = Arrays.asList(SegmentTableModel.CONTENT, SegmentTableModel.SESSION_LINK, SegmentTableModel.CHANNELS);;
    private static final List<String> audioColumnsToSkip = Arrays.asList(SegmentTableModel.CONTENT, SegmentTableModel.SESSION_LINK, SegmentTableModel.RESOLUTION);
    ...
    private String getSheetName(SegmentPanel table, int sheetNumber) {
        String sheetName = table.getVideoStream().getManifest().getVideoName();
        sheetName = StringUtils.isBlank(sheetName) ? "Sheet" + sheetNumber : sheetName.replace("/", ".");
    ...
    public void writeExcel(File file) throws IOException {
        ...
        for (SegmentPanel segmentTable : segmentTables) {
            if (segmentTable.getEnableCheckBox().isSelected()) {
                String sheetName = getSheetName(segmentTable, sheetNumber++);
            ...
            for (SegmentPanel segmentTable : segmentTables) {
                if (segmentTable.getEnableCheckBox().isSelected()) {
                    ...
                    if (videoTable != null) {
                        ...
                        headerData = getHeaderData(videoTable, columnIndicesToSkip, true)
                                            .stream()
                                            .map(String::valueOf)
                    ...
                    if (audioTable != null) {
                        ...
                        // Add header data for video table
                        headerData = getHeaderData(audioTable, columnIndicesToSkip, true)
                                     .stream()
                                     .map(String::valueOf)
                                     .collect(Collectors.joining(","));
                        writer.append(headerData).append(Util.LINE_SEPARATOR);
    ...
    private List<Object> getHeaderData(final JTable table, final List<Integer> columnIndicesToSkip, boolean isCSVExport) {
        // Get Header data row
    ...
    private void populateDataRows(final JTable table, final List<Integer> columnIndicesToSkip, final List<List<Object>> dataRows) {
        // Get data rows
    ...
    private void populateCSVDataRows(JTable table, FileWriter writer, List<Integer> columnIndicesToSkip) throws IOException {
        // Add data rows
