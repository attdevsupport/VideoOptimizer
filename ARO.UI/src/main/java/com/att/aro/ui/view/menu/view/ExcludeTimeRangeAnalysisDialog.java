...
public class ExcludeTimeRangeAnalysisDialog extends JDialog {
	...
	enum DialogItem {
		...
		timerangeanalysis_noResultDataError,
		Button_reset,
		Button_cancel
	}
	...
	private void saveInitTimeRangeValues(double timeRangeStartTime, double timeRangeEndTime) {		
					...
					if (startTime < endTime) {
						if ((startTime >= 0.0) && (startTime <= endTime) &&
							...
							if (!hasDataAfterFiltering(filter)) {
								MessageDialogFactory.getInstance().showErrorDialog(
										ExcludeTimeRangeAnalysisDialog.this,
										ResourceBundleHelper.getMessageString(DialogItem.timerangeanalysis_noResultDataError));
							} else {
