...
public class SegmentPanel extends JPanel implements ActionListener {
	...
	public SegmentPanel(VideoStream videoStream, IARODiagnosticsOverviewRoute diagnosticsOverviewRoute, AROTraceData analyzerResult, SharedAttributesProcesses aroView, VideoManifestPanel videoManifestPanel) {
	...
	public SegmentPanel(boolean manifestFlag, SharedAttributesProcesses aroView, VideoStream videoStream, AROTraceData analyzerResult, VideoManifestPanel videoManifestPanel) {
		streamingVideoData = analyzerResult.getAnalyzerResult().getStreamingVideoData();
		...
		if (videoStream != null && videoStream.getPlayRequestedTime() != null) {
	...
	private void updateHiddenPanelContent(boolean manifestFlag) {
		...
		if (manifestFlag) {
			...
			if (!CollectionUtils.isEmpty(videoStream.getVideoEventList())) {
				...
				videoTablePanel = getStreamTable(VIDEO_TABLE_NAME, videoStream.getVideoEventList(), videoTableScrollPane);
				checkBoxVideo.setSelected(true);
			...
			if (!CollectionUtils.isEmpty(videoStream.getAudioEventList())) {
				...
				audioTablePanel = getStreamTable(AUDIO_TABLE_NAME, videoStream.getAudioEventList(), audioTableScrollPane);
				checkBoxAudio.setSelected(true);
	...
	/**
	 * Currently, this method is only called for a combined view of audio/video tables 
	 * @param jTable
	 */
	...
	@Override
	public void actionPerformed(ActionEvent e) {
		SegmentPanel segmentTable = (SegmentPanel) SwingUtilities.getAncestorOfClass(SegmentPanel.class, (BasicArrowButton) e.getSource());
	...
	public void resizeTableScrollPanes(JScrollPane tableJScrollPane) {
		if (tableJScrollPane != null) {
			if (rowCount > tableHeight) {
				...
				tableJScrollPane.setPreferredSize(new Dimension(tableJScrollPane.getWidth(), streamTables.get(VIDEO_TABLE_NAME).getHeight() + tableHeight));
				tableJScrollPane.getBounds().setSize(tableJScrollPane.getWidth(), streamTables.get(VIDEO_TABLE_NAME).getHeight() + tableHeight);
			}
		}
	}
