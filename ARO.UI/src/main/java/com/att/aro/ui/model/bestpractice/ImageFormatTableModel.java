...
public class ImageFormatTableModel extends DataTableModel<ImageMdataEntry> {
	...
	private static final Logger LOGGER = LogManager.getLogger(ImageFormatTableModel.class.getName());
	...
	protected Object getColumnValue(ImageMdataEntry item, int columnIndex) {
		ImageFormatTableModel.LOGGER.trace(format("getColumnValue, idx:{0}", columnIndex));
		switch (columnIndex) {
		case COL_1:
