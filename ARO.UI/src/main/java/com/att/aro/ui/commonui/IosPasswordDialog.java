...
public class IosPasswordDialog extends JDialog {
	...
	private void initialize(String hint) {
		this.setModal(true);
		String titleName = MessageFormat.format(ResourceBundleHelper.getMessageString("aro.title.short"), 
	...
	private JLabel getJTitleLabel() {
		if (jTitleLabel == null) {
			String labelName = MessageFormat.format(ResourceBundleHelper.getMessageString("Message.entersudopassword"), 
													ApplicationConfig.getInstance().getAppShortName());
			jTitleLabel = new JLabel(labelName, SwingConstants.CENTER);
		}
