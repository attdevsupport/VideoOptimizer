...
public class VideoStallImplTest extends BaseTest {
	...
	private VideoStallImpl videoStallImpl;
	...
	@Before
	public void setup() {
		videoStallImpl = (VideoStallImpl) context.getBean("videoStall");
	...
	public void runTest_no_manifests_THEN_NO_DATA() {
		...
		result = (VideoStallResult) videoStallImpl.runTest(tracedata);
		...
		assertThat(result.getAboutText()).contains("Stalling occurs when either a user’s device or their network cannot keep up with a video file when streaming. This results in a total pause of video playback.");
		assertThat(result.getDetailTitle()).contains("Stalls");
		assertThat(result.getOverviewTitle()).contains("Video: Stalls");
	}
	...
	public void runTest_two_invalid_manifests_selected_THEN_CONFIG_REQUIRED() {
		...
		Mockito.when(manifest.isValid()).thenReturn(false);
