...
public class AndroidImpl implements IAndroid {
	...
	private static final String INTERNAL_DATA_PATH = "/data/data/com.att.android.arodatacollector/";
	...
	public boolean isSDCardEnoughSpace(IDevice device, long kbs) {
		...
		for (String oneLine : shellResult) {
			if ((oneLine.toLowerCase().contains("sdcard")) 
					|| oneLine.toLowerCase().contains("mnt/shell")
					|| oneLine.toLowerCase().contains("storage")	// sdcard is here on marshmallow
					) {
	...
	public boolean checkTcpDumpRunning(IDevice device) {
		...
		/*
		 * For more recent Android OS (Marshmallow and possibly earlier versions),
		 * we need to supply the full path to the tcpdump binary when we use "ps". 
		 * Since we are not certain about the starting OS version with that change, 
		 * we are checking every possible combination of full path|just binary name 
		 * and tcpdump|tcpdump_pie, in the order of the presumably most possible 
		 * working argument to the least possible working argument for "ps". 
		 */
		if (isCollectorRunningInShell(device, TCPDUMP_PIE_PATH) || 
				isCollectorRunningInShell(device, TCPDUMP) ||
				isCollectorRunningInShell(device, TCPDUMP_PATH) ||
				isCollectorRunningInShell(device, TCPDUMP_PIE)) {
	...
	private boolean isCollectorRunningInShell(IDevice device, String cmdArg) {
		boolean isRunning;
		String psCmd = isEmulator(device)?"ps|grep ":"ps ";
		StringBuffer strBuffer = new StringBuffer(psCmd);
		strBuffer.append(cmdArg);
		String[] shellLineResult = getShellReturn(device, strBuffer.toString());
		isRunning = shellOutputContainsCollector(shellLineResult);
		return isRunning;
	}

	private boolean shellOutputContainsCollector(String[] shellOutput) {
