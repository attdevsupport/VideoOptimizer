...
public class RVIStepsDialog extends JDialog {
	...
	private static final String VO_RVI_GUIDE = ResourceBundleHelper.getURLResource("help.userguide.url");

	public RVIStepsDialog(JFrame jFrame) {
		super();
		setTitle(ResourceBundleHelper.getMessageString("Error.rvi.trouble.instruction.title"));
		setModal(true);
		// offset from parent dialog
		Point loc = jFrame.getLocationOnScreen();
		loc.x += 665;
		setLocation(loc);
		...
		JLabel stringLabel = new JLabel(
				MessageFormat.format(ResourceBundleHelper.getMessageString("Error.rvi.trouble.instruction"),
						ResourceBundleHelper.getMessageString("Error.rvi.trouble.instruction.info"),
						ResourceBundleHelper.getMessageString("Error.rvi.trouble.instruction.step1"),
						ResourceBundleHelper.getMessageString("Error.rvi.trouble.instruction.step2"),
						ResourceBundleHelper.getMessageString("Error.rvi.trouble.instruction.step3"),
						ResourceBundleHelper.getMessageString("Error.rvi.trouble.instruction.step4"),
						ResourceBundleHelper.getMessageString("Error.rvi.trouble.instruction.step5")));

		JLabel link = new JLabel(
				ResourceBundleHelper.getMessageString("dlog.collector.option.ios.instruction.learnmore"));
		link.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		link.setForeground(Color.blue);
		link.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				BrowserLauncher bg = new BrowserLauncher();
				bg.launchURI(VO_RVI_GUIDE);
			}
