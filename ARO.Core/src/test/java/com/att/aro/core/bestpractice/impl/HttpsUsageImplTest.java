...
public class HttpsUsageImplTest extends BaseTest {
	...
	public void testParentDomainNameUsed_DifferentSubdomains() {
		...
		pktAnalyzerResult = domainNameTestSetup("157.56.19.80",
												"216.58.194.196",
												"www.gstatic.com", 
												"ssl.gstatic.com",
												"www.arotest.com");
	...
	public void testHttpConnectionsPercentage() {
		...
		if (httpsUsageEntries.get(0).getIPAddress().equals("157.56.19.80")) {
			...
			assertEquals("100", httpsUsageEntries.get(0).getHttpConnectionsPercentage());	
			assertEquals("50", httpsUsageEntries.get(1).getHttpConnectionsPercentage());
		
		} else {
			
			assertEquals("50", httpsUsageEntries.get(0).getHttpConnectionsPercentage());	
			assertEquals("100", httpsUsageEntries.get(1).getHttpConnectionsPercentage());
		}
	}
	...
	public void testHttpTrafficPercentage() {
		...
		if (httpsUsageEntries.get(0).getIPAddress().equals("157.56.19.80")) {
			...
			assertEquals("100", httpsUsageEntries.get(0).getTotalHttpTrafficPercentage());	
			assertEquals("25", httpsUsageEntries.get(1).getTotalHttpTrafficPercentage());
		
		} else {
			
			assertEquals("25", httpsUsageEntries.get(0).getTotalHttpTrafficPercentage());	
			assertEquals("100", httpsUsageEntries.get(1).getTotalHttpTrafficPercentage());
		}
	}
	...
	public void testResultPass() {
		...
		assertEquals(ApplicationConfig.getInstance().getAppShortName() + " discovered 0 non-HTTPS connection and it passes the test.", 
	...
	public void testResultWarn() {
		...
		assertEquals(ApplicationConfig.getInstance().getAppShortName() + " discovered 1 non-HTTPS connections, which is 12.5% of all the TCP sessions.", 
				httpsUsageResult.getResultText());
	}	
	...
	public void testResultFail_NumHttpConnThree() {
		...
		assertEquals(1, httpsUsageEntries.size());
		assertEquals(BPResultType.FAIL, httpsUsageResult.getResultType());
		assertEquals(ApplicationConfig.getInstance().getAppShortName() + " discovered 3 non-HTTPS connections, which is 37.5% of all the TCP sessions.", 
				httpsUsageResult.getResultText());
	}
	...
	public void testResultFail_NumHttpConnMoreThanThree() {
		...
		assertEquals(2, httpsUsageEntries.size());
		assertEquals(BPResultType.FAIL, httpsUsageResult.getResultType());
		assertEquals(ApplicationConfig.getInstance().getAppShortName() + " discovered 4 non-HTTPS connections, which is 50% of all the TCP sessions.", 
				httpsUsageResult.getResultText());
	}
	...
	private PacketAnalyzerResult domainNameTestSetup(String ipAddressA,
								 String ipAddressB,
							     String sessionDomainName1,
							     String sessionDomainName2,
							     String sessionDomainName3) {	
	...
	private PacketAnalyzerResult sessionPacketsPayloadTestSetup(int payloadSize1, int payloadSize2, 
			int payloadSize3, int payloadSize4) {
     ...
     * 
     * Connections 1 - 3 share the same IP.
     * Connections 4 - 7 share the same IP, 
     * different from the one shared by Connections 1 - 3.
     * Connections 8 has a different IP than Connections 1 - 7. 
     * 
     * numHttpConn is expected to be any number between 
     * 1 and 8, inclusively. If it is not, then all 
     * connections/sessions will be set up as HTTPS. 
     * 
     * numHttpConn = 1 - C1 is HTTP Connection
     * numHttpConn = 2 - C1, C2 are HTTP Connections
     * numHttpConn = 3 - C1, C2, C3 are HTTP Connections
     * numHttpConn = 4 - C1, C2, C3, C4 are HTTP Connections
     * ... same logic follows till numHttpConn = 8
     *     
     * Number of TCP Packets in each connection:
     * C1 - 2
     * C2 - 3
     * C3 - 1 
     * C4 - 1 
