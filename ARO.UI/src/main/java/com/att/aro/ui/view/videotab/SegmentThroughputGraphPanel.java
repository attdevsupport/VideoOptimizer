...
import com.att.aro.ui.view.diagnostictab.CreateBarPlot;
...
import com.att.aro.ui.view.videotab.plot.VideoThroughputPlot;
...
public class SegmentThroughputGraphPanel extends JPanel implements ActionListener, ChartMouseListener {
	...
	private VideoThroughputPlot videoThroughputPlot;
	...
	public void refresh(AROTraceData aroTraceData, VideoStream videoStream, JCheckBox checkBoxVideo,
		...
		if (aroTraceData != null && aroTraceData.getAnalyzerResult() != null && aroTraceData.getAnalyzerResult().getSessionlist().size() > 0) {
			...
			if (aroTraceData.getAnalyzerResult().getFilter().getTimeRange().getBeginTime() < aroTraceData
				...
				getYAxis().setRange(Math.round(videoThroughputPlot.getMinYValue() - 0.5),
						Math.round(videoThroughputPlot.getMaxYValue() + 0.5));
			} else {
				getXAxis().setRange(new Range(-0.01, 0));
			}
		}
		videoThroughputPlot.populate(getPlot(), aroTraceData);
