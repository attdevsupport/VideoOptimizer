...
import com.att.aro.ui.view.videotab.plot.VideoProgressPlot;
...
@Data
public class SegmentProgressGraphPanel extends JPanel implements ActionListener, ChartMouseListener {
	...
	private VideoProgressPlot videoProgressPlot;
	 ...
	 * @param videoStream
	 */
	public SegmentProgressGraphPanel(MainFrame aroView) {
	...
	public void refresh(AROTraceData aroTraceData, VideoStream videoStream, JCheckBox checkBoxVideo,
		...
		if (aroTraceData != null && aroTraceData.getAnalyzerResult() != null && aroTraceData.getAnalyzerResult().getSessionlist().size() > 0) {
			...
			videoProgressPlot = new VideoProgressPlot(videoStream, getOptionSelected(checkBoxVideo, checkBoxAudio));

			videoProgressPlot.calculateProgress(getOptionSelected(checkBoxVideo, checkBoxAudio));
			...
			if (aroTraceData.getAnalyzerResult().getFilter().getTimeRange().getBeginTime() < aroTraceData
				...
				getXAxis().setRange(
						new Range(videoProgressPlot.getMinXValue() - 5, videoProgressPlot.getMaxXValue() + 10));

				getYAxis().setRange(Math.round(videoProgressPlot.getMinYValue() - 0.5),
						Math.round(videoProgressPlot.getMaxYValue() + 0.5));
			} else {
				getXAxis().setRange(new Range(-0.01, 0));
			}
		}
		videoProgressPlot.populate(getPlot(), aroTraceData);
