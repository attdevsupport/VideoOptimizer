...
/**
 * IAndroid is an interface to provide information about an Android/Emulator and
 * the controls to perform a trace.
 *
 */
public interface IAndroid {
	...
	boolean pushFile(IDevice emulator, String local, String remote);

	/**
	 * get the device status from ddmlibs library status: BOOTLOADER,OFFLINE,
	 * ONLINE, RECOVERY,UNAUTHORIZED;
	 * 
	 * @param device
	 * @return DeviceState
	 */
	...
	/**
	 * Pull files in emulator pullFile(String remoteFilepath, String
	 * localFilename, ISyncProgressMonitor monitor) throws TimeoutException,
	 ...
	 * @return
	 */
	boolean pullTraceFilesFromEmulator(IDevice device, String remoteFilepath, String localFilename);

	/**
	 * Pull all trace files via DDMLIB [cpu, appid, appname, time,
	 * processed_events, active_process, battery_events, bluetooth_events,
	 * camera_events, device_details, device_info, gps_events, network_details,
	 * prop, radio_events, screen_events, screen_rotations, time,
	 * user_input_log_events, alarm_info_start, alarm_info_end,
	 * batteryinfo_dump, dmesg, video_time, wifi_events, traffic.cap, keys.ssl]
	 * 
	 * @param device
	 * @param remoteFilepath
	 * @param localFilename
	 * @return
	 */
	boolean pullTraceFilesFromDevice(IDevice device, String remoteFilepath, String localFilename);
	 ...
	 * @return
	 */
	boolean makeAROTraceDirectory(IDevice device, String traceName);
	...
	/**
	 * startTcpDump builds a command string based on potential SELinux
	 * environment
	 * 
	 * @param device
	 * @param seLinuxEnforced
	 *            true if need to launch in a SELinux enforced environment
	 ...
	 * 
	 * @return true indicates a trace is in progress, false indicates no trace
	 *         is active
	 */
	 ...
	 */
	boolean removeEmulatorData(IDevice device, String deviceTracePath);
	...
	/**
	 * check if a package exist in the device by issuing command: adb shell pm
	 * list package
	 * 
	 * @param device
