...
public class FileOrderImpl implements IBestPractice {
	...
	public AbstractBestPracticeResult runTest(PacketAnalyzerResult tracedata) {
		...
		if (results.isEmpty()) {
			...
			if (results.size() > 1) {
				...
				text = MessageFormat.format(textResult, 
											ApplicationConfig.getInstance().getAppShortName(), 
											results.size());
			}
		}
	...
	/**
	 * Check element whether they are part of CSS or JS list created from the document.
	 * 
	 */
	...
	/*
	 * Gets the parsed packet and checks any CSS files are getting downloaded after JS.
	 */
	public boolean checkFileOrderAnalysisResults(Document doc, FileOrderResult result) {

		String lastContentType = null;
		ArrayList<Element> docElements = new ArrayList<Element>(doc.getAllElements().size());
		...
		for (Element element : delement) {
			...
			// it is a CSS element.
			if ((checkElementPresentInCSSJSList(element, css, jss)) && (element.tagName().equals("link")) && (element.toString().contains("text/css"))) {
				docElements.add(element);
				continue;
			}
			// Checking whether the element has link and text/css to make sure
			// it is a JS element.
			if ((checkElementPresentInCSSJSList(element, css, jss)) && (element.hasAttr("src")) && (element.tagName().equals("script"))) {
				docElements.add(element);
				continue;
			}
		}
		...
		/*
		 * Updating the last content type after every iteration and checking it to make sure that no CSS files are loaded after a JS
		 */
		for (Element elem : docElements) {
			if ((elem.tagName().equals("link")) && (elem.toString().contains("text/css"))) {
