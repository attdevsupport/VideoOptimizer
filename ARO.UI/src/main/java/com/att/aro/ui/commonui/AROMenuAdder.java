...
/**
 * This allows easy menu item additions with attached listener. Also includes an 'is menu selected' checker.
 * 
 *
 *
 */
public class AROMenuAdder {
	...
	public JCheckBoxMenuItem getCheckboxMenuItemInstance(Enum<?> menuItemKey) {
		JCheckBoxMenuItem helpMenuItem = new JCheckBoxMenuItem(ResourceBundleHelper.getMessageString(menuItemKey));
		helpMenuItem.addActionListener(listener);
		return helpMenuItem;
	}

	public boolean isMenuSelected(Enum<?> key, ActionEvent event) {
		return ResourceBundleHelper.getMessageString(key).equals(event.getActionCommand());
	}
